/**
 * Markdown to Slack Block Kit formatter.
 *
...
function parseMarkdown(markdown: string): Segment[] {
  ...
  while (i < lines.length) {
    // noUncheckedIndexedAccess: i < lines.length guarantees defined
    const line = lines[i]!;
...
function parseTable(tableLines: string[]): TableSegment | null {
  ...
  // noUncheckedIndexedAccess: tableLines.length >= 2 asserted above
...
function buildWideTableBlocks(
  ...
  for (const row of rows) {
    ...
    for (let i = 1; i < headers.length && i < row.length; i++) {
      // noUncheckedIndexedAccess: i < headers.length and i < row.length
