...
export function buildTeamBuilderPrompt(options: RolePromptOptions): string {
  ...
  return [
    ...
    '',
    '   **CRITICAL node-naming convention**: the `nodes:` map in',
    '   workflow.yaml MUST have keys that match the script filenames',
    '   (without `.ts`). If your file is `hello-world.ts`, the node key',
    '   must be `hello-world`, NOT `greet` or anything else. tntc',
    '   generates a ConfigMap keyed by filename; if the node name',
    '   diverges, the pod fails to start with',
