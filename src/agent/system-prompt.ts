...
function buildIdentityContext(userSlackId: string, userEmail: string): string {
  return [
    ...
    'Check-before-use: before every tntc or MCP tool call, read expires_at from',
    '  token.json. If (expires_at - $(date +%s)) < 900, re-read the file — the',
    '  bridge refreshes token.json between turns so a fresh read is always valid.',
    'Never hold a token in memory across tool calls — always read fresh from the file.',
...
export function buildManagerPrompt(
  ...
  const sections: string[] = [
    ...
    '- Write a commission_dev_team signal to signals-out.ndjson with a concrete task spec.',
    ...
    '  signals from the dev team in signals-in.ndjson, emitting heartbeats as appropriate.',
    '',
    '### When in doubt',
    ...
    '- Read signals-in.ndjson for dev team progress updates',
    '- Write commission_dev_team / terminate_dev_team signals to signals-out.ndjson',
...
export function buildBuilderPrompt(
  ...
  return [
    ...
    '',
    '## When Done',
    '- Write a task_completed signal to signals-in.ndjson',
    '- Exit cleanly',
    '',
    buildIdentityContext(userSlackId, userEmail),
  ].join('\n');
...
export function buildDeployerPrompt(
  ...
  return [
    ...
    '5. Write task_completed signal to signals-in.ndjson',
    '',
    '## Tools Available',
